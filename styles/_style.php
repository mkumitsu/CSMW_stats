<?php
	...
	$settings = Array(
		...
		"mode" => "sql",              // Modus des CSS-Wrappers (array, file, sql)
	...
	if ($settings['mode'] == "array") {
		...
		$func = new Functions();
		$values = $func->load_settings("style");
	}
	...
	$css = Array(
		...
		"transform"     => Array("-moz-transform", "-webkit-transform", "-o-transform", "-ms-transform", "transform")
	);
		...
		if (file_exists($settings['path'].trim($style).'.css')) {
			...
			foreach ($file as $line) {
				...
				if (strpos($line, "§") !== FALSE) {                                        // Check ob in einer Zeile ein "§" vorhanden ist
					...
					$line = isset($values[$line]) ? $values[$line] : "";
				}
