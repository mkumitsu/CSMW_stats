<?php
	...
	if (true) {
		...
		$clan = (isset($_GET['clan'])) ? $_GET['clan'] : "No clanname given";
		$preset['settings_clanname'] = htmlspecialchars($clan);
			...
			while ($row = $sql->fetchAssoc()) {
				...
				if ($ov['name']) {
					...
					$data['player'][$i][$column]  = html(Array(
						'sort' =>   $nname,
						'format' => '<a target="_blank" href="'.$url[$region].'/community/accounts/'.$aid.'">'.$clantag.$nname.'</a>',
				...
				if ($ov['xp']) {
					$column = 'xp';
					$result = $stats->get('statistics', $column, Array('number'), false);
				...
				if ($ov['battle_avg_xp']) {
					...
					$battles = $stats->get('statistics', 'battles');
					$result  = $stats->get('statistics', 'xp', Array('ratio', $battles, 0), false);
				...
				if ($ov['frags_per_battle']) {
					...
					$battles = $stats->get('statistics', 'battles');
					$frags   = $stats->get('statistics', 'frags');
					$result  = $stats->form($frags->total, Array('ratio', $battles, 2), false);
				...
				if ($ov['hits_percents']) {
					$column = 'hits_percents';
					$shots  = $stats->get('statistics', 'shots');
					$hits   = $stats->get('statistics', 'hits');
					$result = $stats->form($hits->total, Array('ratio', $shots->total, 0, '%'), false);
				...
				if ($ov['damage_dealt']) {
					$column = 'damage_dealt';
					$result = $stats->get('statistics', $column);
					$result = $stats->form($result->total, Array('number', 0, "k"), false);
				...
				if ($ov['wlr']) {
					$column = 'wlr';
					$loss  = $stats->get('statistics', 'losses');
					$result = $stats->get('statistics', 'wins', Array('ratio', $loss, 2), false);
